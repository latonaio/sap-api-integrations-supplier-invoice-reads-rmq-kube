...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-supplier-invoice-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(supplierInvoice, fiscalYear string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "SupplierInvoiceHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) Tax(supplierInvoice, fiscalYear string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "SupplierInvoiceTax"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) Account(supplierInvoice, fiscalYear string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "SupplierInvoiceAccountAssignment"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) PurchaseOrder(purchaseOrder, purchaseOrderItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "SupplierInvoicePurchaseOrderReference"})
	if err != nil {
		c.log.Error(err)
		return
	}
